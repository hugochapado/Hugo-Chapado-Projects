...
class linkedlist{
	...
	public:
		...
		void deletenode(Node* root,std::string  id);
...
Node* linkedlist::insert(Node* root, std::string  id ){
...
void linkedlist::deletenode(Node* root, std::string id){
	Node* temp = root;
	Node* temp2 = NULL;
	if(root == NULL){  //Empty List
		return;
	}
	else if(root->next == NULL){ //Only the root
		root=NULL;
		delete temp;
        size--;
		return;
	}						
	while(temp != NULL){		//More than 1 node list
		if (temp->id == id){
...
Node* linkedlist::search(Node* root, std::string  id){ 
    Node* temp = root;
    while (temp != NULL)
    {
        if (temp->id == id)
...
std::string& UnorderedMap::operator[] (std::string const& key) 
{
    ...
    Node* node =map->at(pos)->search(map->at(pos)->getroot() , id);
    if (node != nullptr){ //Search
        return node->name;
    }
    else{ //Insert
        Node* node = map->at(pos)->insert(map->at(pos)->getroot() , id);
        if(loadFactor() >= maxLoadFactor ){ //check if reash
            rehash();
        }   
        keys++;
        return node->name;
...
void UnorderedMap::remove(std::string const& key) 
{
    ...
    const char* convertedKey = key.c_str();
    unsigned int pos = hashFunction(convertedKey,bCount);  
    map->at(pos)->deletenode(map->at(pos)->getroot(),id);
    keys--;
}
...
{
    int factor = bCount * size();
    double loadfactor = factor;
	return  loadfactor;
}
